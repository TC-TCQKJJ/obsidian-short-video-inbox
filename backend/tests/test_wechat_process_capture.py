...
class ProcessCaptureSourceBoundaryTest(unittest.TestCase):
    def test_go_helper_loads_unique_ca_without_public_listener_or_system_proxy(self):
        ...
        self.assertIn("createCaptureCertificate(", source)
        self.assertIn("rsa.GenerateKey(rand.Reader, 2048)", source)
        self.assertIn("manager.PrivateKey = string(leafKeyPEM)", source)
    ...
    def test_build_removes_and_verifies_absence_of_upstream_private_key(self):
        ...
        self.assertIn("targetIP.IsLoopback()", patch_source)
        self.assertIn("r.Target.Parse(res.Host, 443)", patch_source)
